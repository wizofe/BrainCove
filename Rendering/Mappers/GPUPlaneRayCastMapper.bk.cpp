...
GPUPlaneRayCastMapper::GPUPlaneRayCastMapper()
{
	...
	this->RayCaster				 = NULL;
	//this->RayCaster				 = new CLRayCaster("raycastPlane");

	this->Correlation			 = 0.0;
	this->NewVoxelPicked		 = false;
	this->VoxelPicked			 = 0;
...
void GPUPlaneRayCastMapper::ReleaseGraphicsResources(vtkWindow *)
{
}

void GPUPlaneRayCastMapper::SetRayCaster(CLRayCaster* rayCaster)
{
...
void GPUPlaneRayCastMapper::SetLayout(BrainLayout1D *layout)
{
	...
	this->RayCaster->SetLayout(layout);

}

void GPUPlaneRayCastMapper::Render( vtkRenderer *ren, vtkVolume *vol )
{
	...
	{
		SelectionMap map;
		if(this->NewVoxelPicked)
		{
			this->VoxelPicked = PickVolume(this->MouseCoordinates[0],this->MouseCoordinates[1],ren);
			map.insert(SelectionEntry(this->VoxelPicked,1));
		}
		//Initialize OPENCL kernel
		//Render to texture!
		RayCaster->CastRays(Texture,ViewToVoxelsMatrix,this->ImageOrigin,this->ImageViewportSize,cam->GetThickness(), this->RowBounds, map, this->Correlation); 
...
double GPUPlaneRayCastMapper::GetZBufferValue(int x, int y)
{
	int xPos, yPos;
...
void GPUPlaneRayCastMapper::ComputeVolume()
{
	if(this->ImageData)
		this->ImageData->Delete();

	this->ImageData = vtkImageData::New();
	this->ImageData->SetScalarTypeToInt();
	this->ImageData->SetExtent(0,99,0,99,0,1);
	this->ImageData->AllocateScalars();

	int* dataPtr = static_cast<int*>(this->ImageData->GetScalarPointer());
	int nPts = this->ImageData->GetNumberOfPoints();
	...
	this->SetInput(this->ImageData);

	this->RayCaster->SetVolumeData(this->ImageData);
}

void GPUPlaneRayCastMapper::NormalizeCoordinates(int x, int y, float z, float w, int *viewPortSize, float outCoord[4])
{
...
int GPUPlaneRayCastMapper::PickVolume(int x, int y, vtkRenderer *renderer)
{
	this->NewVoxelPicked = false;
	...
	for(int i = 0; i < num_steps && voxel == -1 && !this->NewVoxelPicked; i++)
	{
...
int GPUPlaneRayCastMapper::ComputeRowBounds(vtkVolume   *vol,
											vtkRenderer *ren)
{
	float voxelPoint[3];
	float viewPoint[8][4];
...
void GPUPlaneRayCastMapper::ComputeMatrices( vtkImageData *data, 
											vtkVolume *vol)
{
	...
	vtkMatrix4x4::DeepCopy(*this->VolumeMatrix->Element, *vol->GetMatrix()->Element);
...
double *GPUPlaneRayCastMapper::GetBounds()
{
...
void GPUPlaneRayCastMapper::SetConnectivityMatrix(BaseMatrix<float> *matrix)
{
	inputMatrix = matrix;
	if(!this->RayCaster)
		this->RayCaster = new CLRayCaster("raycastPlane");
	this->RayCaster->SetMatrix(matrix);
...
void GPUPlaneRayCastMapper::SetMouseCoordinates(int x, int y)
{
	MouseCoordinates[0] = x;
	MouseCoordinates[1] = y;

	this->NewVoxelPicked = true;

	this->Modified();
}

void GPUPlaneRayCastMapper::SetCorrelation(double corr)
{
...
BaseMatrix<float> *GPUPlaneRayCastMapper::GetConnectivityMatrix()
{
	return inputMatrix;
}

// Print method for vtkVolumeRayCastMapper
void GPUPlaneRayCastMapper::PrintSelf(ostream& os, vtkIndent indent)
{
	this->Superclass::PrintSelf(os,indent);

}

/*int GPUPlaneRayCastMapper::FillInputPortInformation(int port, vtkInformation* info)
{
