...
#include <Layouts/BrainVoxelSelector.h>
...
public:
  ...
  SelectionMap GetVoxelsPicked() const;

  virtual void SetVoxelCoordinates(int x, int y, int z);
  virtual void GetVoxelCoordinates(int xyz[3]);
...
protected:
  ...
  std::map<int,float> PickVolume(int x, int y, vtkRenderer *render);
