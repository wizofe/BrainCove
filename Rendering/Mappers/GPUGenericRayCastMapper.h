...
{
public:
	vtkTypeMacro(GPUGenericRayCastMapper,vtkVolumeMapper);

	void PrintSelf( ostream& os, vtkIndent indent );

	virtual void SetLayoutSystem(BaseLayout *layout) 
	{ 
		this->Layout = layout; 
		this->RayCaster->SetLayout(layout);
	}

	virtual void SetConnectivityData( DataFile *file) { MatrixFile = file; }
	virtual void SetComparisonData( DataFile *file);

	virtual void SetRenderModeToComparison() { this->RayCaster->SetRenderModeToComparison(); }
	virtual void SetRenderModeToNormal() { this->RayCaster->SetRenderModeToNormal(); }

	virtual DataFile *GetConnectivityData() { return MatrixFile; }
	...
	virtual vtkImageData* GetMaskVolume() = 0;
	virtual vtkImageData* GetAtlasVolume() = 0;

	virtual void SetAtlasVolume(vtkImageData* data) { };
	virtual void SetMaskVolume(vtkImageData* data) { };

	// Description:
	// This is the actual program that will run to draw pixels on the texture
	virtual void SetRayCaster(CLRayCaster* rayCaster);
	virtual CLRayCaster* GetRayCaster() { return RayCaster; }
	...
	vtkMatrix4x4 *GetVolumeMatrix( vtkImageData *data, vtkVolume *act );

	void Render( vtkRenderer *, vtkVolume * );
	void ReleaseGraphicsResources(vtkWindow *);

	vtkMatrix4x4 *GetViewMatrix() { return this->ViewToWorldMatrix; }
...
protected:
  ...
  vtkTransform *WorldToViewTransform;
  ...
  DataFile *MatrixFile;
  DataFile *CompareMatrixFile;

  virtual void UpdateRaycaster(vtkVolume *vol) = 0;
  //virtual void UpdatePicker(vtkVolume *vol) = 0;
  ...
  //BaseVoxelSelector *VoxelSelector;

  //std::vector<GPUGenericRayCastMapper*> Observers;
...
private:
  GPUGenericRayCastMapper(const GPUGenericRayCastMapper&);  // Not implemented.
  void operator=(const GPUGenericRayCastMapper&);  // Not implemented.
